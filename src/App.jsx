...
const CoachDashboard = ({ user, db, setDb, onLogout }) => {
  ...
  const addEvent = () => { if (!newEvent.date || !newEvent.time || !newEvent.location) return; const e = { id: uuid(), teamId: user.teamId, type: newEvent.type, title: newEvent.type === 'game' ? (newEvent.title || 'Game') : 'Practice', date: newEvent.date, time: newEvent.time, location: newEvent.location, field: '', arriveBy: newEvent.time, needsSnacks: newEvent.type === 'game' }; const newDuties = newEvent.type === 'game' ? [{ id: uuid(), eventId: e.id, type: 'snacks', assignedTo: null, assignedId: null }, { id: uuid(), eventId: e.id, type: 'drinks', assignedTo: null, assignedId: null }] : []; setDb({ ...db, events: [...db.events, e], duties: [...db.duties, ...newDuties] }); setNewEvent({ type: 'practice', title: '', date: '', time: '', location: '' }); setShowAddEvent(false); setToast({ type: 'game', title: 'Event Created', message: `${e.title} on ${formatDate(e.date)}` }); };
